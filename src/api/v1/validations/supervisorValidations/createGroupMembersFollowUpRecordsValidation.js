...
const createGroupMembersFollowUpRecordsSchema = Joi.object({
  group_plan_id: Joi.number().integer().min(1).required().messages({
    "number.base": "Group Plan ID must be a number.",
    "number.min": "Group Plan ID must be at least 1.",
    "any.required": "Group Plan ID is required.",
  }),
...
const validateCreateGroupMembersFollowUpRecords = (req, res, next) => {
  console.log("validate day date");

  console.dir(req.body, { depth: null });
  const { error } = createGroupMembersFollowUpRecordsSchema.validate(req.body, {
    abortEarly: false,
  });
...
module.exports = validateCreateGroupMembersFollowUpRecords;
